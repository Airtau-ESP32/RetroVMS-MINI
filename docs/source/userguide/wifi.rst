...
The RetroVMS WiFi system is based on the ESP32 integrated WiFi transceiver. It uses and provides WiFi 
...
The RetroVMS WiFi network can be configured as a client (connecting to existing WiFi networks) and/or 
...
time in milliseconds using the config command like this, e.g. to 200 milliseconds::
  ...
  RetroVMS# config set network wifi.scan.tmin 200
...
first one. Do so by setting ``tmax``, e.g.::
  ...
  RetroVMS# config set network wifi.scan.tmax 300
...
variables::
  ...
  RetroVMS# config set network wifi.sq.good -87.0
  RetroVMS# config set network wifi.sq.bad -89.0
...
to true, either using the web UI or by doing::
  ...
  RetroVMS# config set network wifi.bad.reconnect yes
...
To connect with a **static client address** instead of using DHCP, use the ``wifi ip static`` command::
  ...
  RetroVMS# wifi ip static [<ip> <subnet> <gateway>]
...
configuration syntax::
  ...
  RetroVMS# config set wifi.ssid "<ssid>.RetroVMS.staticip" "<ip>,<subnet>,<gateway>"
...
third argument to the ``wifi mode apclient`` command::
  ...
  RetroVMS# wifi mode client <ssid> <bssid>

  RetroVMS# wifi mode apclient <apssid> <ssid> <bssid>
...
bandwidth is preferred, eg for high throughput logging, set the network configuration ``wifi.ap.bw`` with the command::
  ...
  RetroVMS# config set network wifi.ap.bw 40 
