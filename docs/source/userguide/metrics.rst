...
Metrics are at the heart of the RetroVMS v3 system. They are strongly typed named 
...
The full list of metrics available can be shown::
  ...
  RetroVMS# metrics list
  m.freeram                                4232852
  m.hardware                               RetroVMS WIFI BLE BT cores=2 rev=ESP32/1
  m.monotonic                              3568Sec
  ...
...
A base RetroVMS v3 system has more than 100 metrics available (see below), and
...
m.hardware                               RetroVMS WIFI BLE BT…        Base module hardware info
...
.. code-block:: javascript
  ...
  (function(){
    ...
    // Check vehicle state:
    if (!RetroVMSMetrics.Value("v.e.on")) return;

    // Transmit metrics:
    var metrics = RetroVMSMetrics.GetValues(cfg.metricnames);
    var msg = cfg.recordtype + ",0," + cfg.expiretime + "," + Duktape.enc('jc', metrics);
    RetroVMSNotify.Raise("data", cfg.notifytype, msg);
