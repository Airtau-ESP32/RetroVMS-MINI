...
RetroVMS commands. A command script can be executed with::

  RetroVMS# . <script>
  RetroVMS# script run <script>
...
Note that the developer building firmware can optionally set the ``RetroVMS_DEV_SDCARDSCRIPTS`` build 
...
RetroVMS v3 includes a powerful JavaScript engine. In addition to the standard, relatively fixed, 
...
.. note:: The scripting engine used is `Duktape <https://duktape.org/>`_. Duktape supports 
  ...
  or document objects etc., just core Javascript plus the RetroVMS API and plugins.
...
to reload it, you can with::
  ...
  RetroVMS# script reload

------------------
JavaScript Modules
------------------

The RetroVMS JavaScript engine supports the concept of modules (using the node.js style of exports). 
...
To automatically load a custom module on startup, add the ``MyPlugin = require("lib/MyPlugin");`` line to ``RetroVMSmain.js``.
...
A number of RetroVMS objects have been exposed to the JavaScript engine, and are available for use by custom
...
method::
  ...
  RetroVMS# script eval 'JSON.print(this)'
  {
    "performance": {
...
CBOR also isn't part of the standard browser builtin Javascript APIs yet, so the RetroVMS
...
- ``req = HTTP.Request(cfg)``
    ...
    - ``headers``: optional array of objects containing key-value pairs of request headers.
      Note: ``User-Agent`` will be set to the standard RetroVMS user agent if not present here.
        ...
        url: "http://solarcontroller.local/status?fmt=json",
          ...
          if (resp.statusCode == 200) {
            ...
            if (status["power"] > 5000)
              RetroVMSVehicle.StartCharge();
            else if (status["power"] < 3000)
              RetroVMSVehicle.StopCharge();
          }
        }
      });
...
Loading or saving protected paths (``/store/RetroVMS_config/…``) is not allowed. Saving to
...
.. warning::
  **File I/O, especially saving, can cause short freezes of the module!**
  ...
  .. code-block:: javascript
    ...
    function allowSave() {
      return !RetroVMSMetrics.Value("v.e.on") && !RetroVMSMetrics.Value("v.c.charging");
    }
...
- ``id = PubSub.subscribe(topic, handler)``
    ...
    RetroVMS events. The method returns an ``id`` to be used to unsubscribe the handler.
...
- ``str = RetroVMSCommand.Exec(command)``
    The RetroVMSCommand object exposes one method “Exec”. This method is passed a single parameter as the command
    to be executed, runs that command, and then returns the textual output of the command as a string. For
    example::

      print(RetroVMSCommand.Exec("boot status"));
...
- ``array = RetroVMSConfig.Params()``
    Returns the list of available configuration parameters.
- ``array = RetroVMSConfig.Instances(param)``
    Returns the list of instances for a specific parameter.
- ``string = RetroVMSConfig.Get(param,instance,default)``
    Returns the specified parameter/instance value.
- ``object = RetroVMSConfig.GetValues(param, [prefix])``
...
- ``RetroVMSConfig.Set(param,instance,value)``
    Sets the specified parameter/instance value.
- ``RetroVMSConfig.SetValues(param, prefix, object)``
...
- ``RetroVMSConfig.Delete(param,instance)``
...
``RetroVMSConfig.Set("usr", "myplugin.level", 123)`` or by the ``config set`` command.
...
.. code-block:: javascript
  ...
  Object.assign(cfg, RetroVMSConfig.GetValues("usr", "myplugin."));
...
This provides access to the RetroVMS event system. While you may raise system events, the primary use is to raise
...
.. code-block:: javascript
  ...
  // start timeout:
  RetroVMSEvents.Raise('usr.myplugin.timeout', 1500);

- ``RetroVMSEvents.Raise(event, [delay_ms])``
...
- ``isatlocation = RetroVMSLocation.Status(location)``
...
- ``num = RetroVMSMetrics.AsFloat(metricname [,unitcode])``
    ...
    Mismatched ``unitcode`` will be ignored.
- ``str = RetroVMSMetrics.AsJSON(metricname)``
    Returns the JSON representation of the metric value.
- ``obj = RetroVMSMetrics.GetValues([filter] [,unitcode] [,decode])``
    ...
    For ``RetroVMSMetrics.Value`` and ``RetroVMSMetrics.GetValues`` if a ``unitcode`` is specified
...
.. code-block:: javascript
  ...
  var speed  = RetroVMSMetrics.Value("v.b.range.speed", "native", false)

With the introduction of the ``RetroVMSMetrics.GetValues()`` call, you can get multiple metrics
...
.. code-block:: javascript
  ...
  var metrics = RetroVMSMetrics.GetValues("v.b.c.");
  ...
  var RetroVMSinfo = RetroVMSMetrics.GetValues(["m.version", "m.hardware"]);
  JSON.print(RetroVMSinfo);
...
.. code-block:: javascript
  ...
  var celltemps = eval(RetroVMSMetrics.AsJSON("v.b.c.temp"));
...
- ``id = RetroVMSNotify.Raise(type, subtype, message)``
    ...
    .. code-block:: javascript
      ...
      RetroVMSNotify.Raise("info", "usr.myplugin.status", "Alive and kicking!");

      // send a JSON stream to a web plugin:
      RetroVMSNotify.Raise("stream", "usr.myplugin.update", JSON.format(streamdata, false));

      // send a CSV data record to a server:
      RetroVMSNotify.Raise("data", "usr.myplugin.record", "*-MyStatus,0,86400,Alive");


RetroVMSVehicle
^^^^^^^^^^^

The RetroVMSVehicle object is the most comprehensive, and exposes several methods to access the current vehicle. These include:

- ``str = RetroVMSVehicle.Type()``
    Return the type of the currently loaded vehicle module
- ``success = RetroVMSVehicle.Wakeup()``
    Wakeup the vehicle (return TRUE if successful)
- ``success = RetroVMSVehicle.Homelink(button, durationms)``
    Fire the given homelink button
- ``success = RetroVMSVehicle.ClimateControl(onoff)``
    Turn on/off climate control
- ``success = RetroVMSVehicle.Lock(pin)``
    Lock the vehicle
- ``success = RetroVMSVehicle.Unlock(pin)``
    Unlock the vehicle
- ``success = RetroVMSVehicle.Valet(pin)``
    Activate valet mode
- ``success = RetroVMSVehicle.Unvalet(pin)``
    Deactivate valet mode
- ``success = RetroVMSVehicle.SetChargeMode(mode)``
    Set the charge mode ("standard" / "storage" / "range" / "performance")
- ``success = RetroVMSVehicle.SetChargeCurrent(limit)``
    Set the charge current limit (in amps)
- ``success = RetroVMSVehicle.SetChargeTimer(onoff, start)``
    Set the charge timer
- ``success = RetroVMSVehicle.StartCharge()``
    Start the charge
- ``success = RetroVMSVehicle.StopCharge()``
    Stop the charge
- ``success = RetroVMSVehicle.StartCooldown()``
    Start a cooldown charge
- ``success = RetroVMSVehicle.StopCooldown()``
    Stop the cooldown charge

- ``result = RetroVMSVehicle.ObdRequest(arguments)``
    ...
    **Example**:
    ...
    .. code-block:: javascript
      ...
      var res = RetroVMSVehicle.ObdRequest({ txid: 0x765, rxid: 0x7cf, request: "1003" });
      if (res.error)
        print(res.errortext);
      else
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
RetroVMSVehicle Command Plugins
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Most vehicles do not implement all standard vehicle commands. When trying to execute
one of these, the system will respond with "not implemented". Using the ``RetroVMSVehicle``
object, you can register your own handlers for these. This applies to the following
functions:

- ``RetroVMSVehicle.Wakeup()``
- ``RetroVMSVehicle.Homelink(button, durationms)``
- ``RetroVMSVehicle.ClimateControl(onoff)``
- ``RetroVMSVehicle.Lock(pin)``
- ``RetroVMSVehicle.Unlock(pin)``
- ``RetroVMSVehicle.Valet(pin)``
- ``RetroVMSVehicle.Unvalet(pin)``
- ``RetroVMSVehicle.SetChargeMode(mode)``
- ``RetroVMSVehicle.SetChargeCurrent(limit)``
- ``RetroVMSVehicle.SetChargeTimer(onoff, start)``
- ``RetroVMSVehicle.StartCharge()``
- ``RetroVMSVehicle.StopCharge()``
- ``RetroVMSVehicle.StartCooldown()``
- ``RetroVMSVehicle.StopCooldown()``
...
function to the respective ``RetroVMSVehicle`` property. Your custom function shall
...
.. code-block:: javascript
  ...
  RetroVMSVehicle.Homelink = function(button, durationms) {
    ...
    HTTP.Request({
      ...
      fail: function() { const msg = fndesc + " FAILED: " + this.error; print(msg);
        if (notify) RetroVMSNotify.Raise("alert", "homelink", msg); },
...
To load this plugin automatically on boot, add the code to ``RetroVMSmain.js``, either
...
.. code-block:: bash
  ...
  RetroVMS_HOST="yourRetroVMS.local"

  SCP="/usr/bin/scp -q"
  SSH="/usr/bin/ssh"

  # Upload:
  $SCP "${FILE}" "${RetroVMS_HOST}:${PATH}${FILE}"

  # Execute:
  $SSH "${RetroVMS_HOST}" "script run ${FILE}"
...
A faster option is using the RetroVMS HTTP REST API. The following script uses ``curl`` to upload
...
.. code-block:: bash
  ...
  RetroVMS_HOST="http://yourRetroVMS.local"
  RetroVMS_USER="admin"
  RetroVMS_PASS="yourpassword"
  ...
  if [[ "$AUTHAGE" -ge 3600 ]] ; then
    RES=$($CURL "${RetroVMS_HOST}/login" --data-urlencode "username=${RetroVMS_USER}" --data-urlencode "password=${RetroVMS_PASS}" 2>/dev/null)
    if [[ "$RES" =~ "Error" ]] ; then
      echo -n "LOGIN ERROR: "
  ...
  # Upload:
  RES=$($CURL "${RetroVMS_HOST}/edit" --data-urlencode "path=${PATH}${FILE}" --data-urlencode "content@${FILE}" 2>/dev/null)
  if [[ "$RES" =~ "Error" ]] ; then
    echo -n "UPLOAD ERROR: "
    echo $RES | $SED -e 's:.*<li>\([^<]*\).*:\1:g'
    rm .auth
  ...
  # Execute:
  $CURL "${RetroVMS_HOST}/api/execute" --data-urlencode "command=script run ${FILE}"
