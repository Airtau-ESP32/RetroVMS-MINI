...
RetroVMS v3 includes a powerful command line console that can be accessed in various ways:

1. Using a micro USB cable to a host computer.

  If the RetroVMS is not recognised via USB download the driver from `SILABS website <https://www.silabs.com/products/development-tools/software/usb-to-uart-bridge-vcp-drivers>`_. You will also need a suitable terminal emulator. The baud rate is 115200, and you should not enable hardware flow control.
...
5. Remote Apps.
  ...
  The RetroVMS Android App currently includes a shell screen that can be used to issue command line console commands via the RetroVMS server v2. This functionality is not currently available in the iPhone/iPad App.
...
5. Once you have established the connection, press ENTER to see the “RetroVMS>” prompt.

^^^^^^^^^^^
SSH Console
^^^^^^^^^^^

A workstation (Mac, Linux, Window), on the same wifi network as the RetroVMS module, can use the ssh protocol to connect. In Windows you can use the free PUTTY ssh client. In Linux and OSX ssh is built-in.

The syntax is simply:

  ``ssh user@ip``

Where ‘user’ is the username (normally ‘RetroVMS’) and ‘ip’ is the IP address of the RetroVMS v3 module. In environments supporting mDNS networking, you should also be able to connect using the mDNS name *<vehicleid>.local*. The password you enter is the module password.

If you use ssh public/private key pairs, you can store your public key on the RetroVMS v3 module, to take advantage of passwordless login.

  ``RetroVMS# config set ssh.keys <user> <public-key>``
...
You can also use SCP to copy files to and from the RetroVMS v3 VFS.
...
Let’s use SSH to demonstrate this::
  ...
  $ ssh RetroVMS@RetroVMS.local

  Welcome to the Open Vehicle Monitoring System (RetroVMS) - SSH Console
  Firmware: 3.1.003-2-g7ea18b4-dirty/factory/main
  Hardware: RetroVMS WIFI BLE BT cores=2 rev=ESP32/1

  RetroVMS#

When first connecting using USB, the console will be in non-secure mode (as indicated by the “RetroVMS>” prompt). Here, only a limited number of commands are available (such as viewing network status, modem status, or system time). To get to secure mode, enter the command ‘enable’, and provide the module password. The prompt will then change to “RetroVMS#”::
...
At any time, you can use “?” to show the available commands. For example::
  ...
  server               RetroVMS Server Connection framework
